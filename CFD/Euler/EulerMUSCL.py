...
while (not is_stop):
    ...
    # 计算边界上的值
    UL_boundary = BD.boundary_value(U_data[:,0],edge_mark[0])
    UR_boundary = BD.boundary_value(U_data[:,-1],edge_mark[-1])
    # reconstruction P1 function 重构部分 均匀一致的网格可以快速实现
    DeltaPu =np.zeros([DIM+2,n_ele])
    DeltaMu = np.zeros([DIM + 2, n_ele])
    DeltaPu[:,:-1]=U_data[:,1:]-U_data[:,:-1]
    DeltaMu[:,1:] = U_data[:, 1:] - U_data[:,:-1]
    DeltaPu[:,-1]=UR_boundary-U_data[:,-1]
    DeltaMu[:,0]=U_data[:,0]-UL_boundary
    DeltaPu = DeltaPu/ele_vol
    DeltaMu = DeltaMu/ele_vol
    # 限制器
    '''
    minmod
    '''
    DeltaU=np.minimum(np.abs(DeltaPu),np.abs(DeltaMu))*((DeltaPu*DeltaPu)>0)
    DeltaU=DeltaU*np.sign(DeltaMu)
    # compute egde value and flux
    for i in range(n_edge):
        if edge2cell[i, 0] == -1:
            Ur = U_data[:,edge2cell[i, 1]]-0.5*ele_vol*DeltaU[:,edge2cell[i, 1]]
            Ul = UL_boundary
        elif edge2cell[i, 1] == -1:
            Ul = U_data[:,edge2cell[i, 0]]+0.5*ele_vol*DeltaU[:,edge2cell[i, 0]]
            Ur = UR_boundary
        else:
            Ul = U_data[:,edge2cell[i, 0]]+0.5*ele_vol*DeltaU[:,edge2cell[i, 0]]
            Ur = U_data[:,edge2cell[i, 1]]-0.5*ele_vol*DeltaU[:,edge2cell[i, 1]]
...
plt.legend()
plt.ylabel(r"$\rho$")
plt.savefig("rhoMUSCL.png",dpi=300)
plt.clf()
...
plt.legend()
plt.ylabel(r"$u$")
plt.savefig("uMUSCL.png",dpi=300)
plt.clf()
...
plt.legend()
plt.ylabel(r"$p$")
plt.savefig("pMUSCL.png",dpi=300)
plt.clf()