...
def updata_val_backup3_2():
    temp_IC_v = np.sum(phase_vol_cp[:,IC_idx_new-1:IC_idx_new+2],axis=1)
    temp_IC_c = np.sum(phase_centroid_cp[:,IC_idx_new-1:IC_idx_new+2]*phase_vol_cp[:,IC_idx_new-1:IC_idx_new+2],axis=1)/temp_IC_v
    temp_IC_v_n = np.sum(phase_vol_new[:,IC_idx_new-1:IC_idx_new+2],axis=1)
    temp_IC_c_n = np.sum(phase_centroid_new[:,IC_idx_new-1:IC_idx_new+2]*phase_vol_new[:,IC_idx_new-1:IC_idx_new+2],axis=1)/temp_IC_v_n
    for phase in range(2):
        Uf = np.zeros([DIM+2,M_b,3],dtype=float)
        for i in range(IC_idx_new-1,IC_idx_new+2):
            if phase_vol_cp[phase,i] > eps:
                x_int_sc = 0.5 * phase_vol_cp[phase,i] * x_int_ref + phase_centroid_cp[phase,i]
                ele_bias_sc = bais(x_int_sc, phase_centroid_cp[phase,i], phase_vol_cp[phase,i])
                ele_bias_sc_IC = bais(x_int_sc, temp_IC_c[phase], temp_IC_v[phase])
                Uf[:,:,i-(IC_idx_new-1)] = (U_data_cp[:,:,i,phase] @ ele_bias_sc) @ np.transpose(ele_bias_sc_IC*wi) * phase_vol_cp[phase,i]
        x_int_IC = 0.5 * temp_IC_v[phase] * x_int_ref + temp_IC_c[phase]
        ele_bias_IC = bais(x_int_IC, temp_IC_c[phase], temp_IC_v[phase])
        M = (ele_bias_IC * wi) @ np.transpose(ele_bias_IC) * temp_IC_v[phase]
        U_IC_cp[:,:,phase] = np.sum(Uf,axis=2)/np.diagonal(M)
...
def update_interface_val3_2(dt,dU_ic):
    # 三阶格式的第二步
    for phase in range(2):
        x_int_IC = 0.5 * IC_vol[phase] * x_int_ref + IC_centroid[phase]
        ele_bias_IC = bais(x_int_IC, IC_centroid[phase], IC_vol[phase])
        ...
        M = (ele_bias_IC * wi) @ np.transpose(ele_bias_IC) * IC_vol[phase]
        M_n = (ele_bias_IC_n * wi) @ np.transpose(ele_bias_IC_n) * IC_vol_new[phase]
        U_IC[:,:,phase] =((3/4)*U_IC_cp[:,:,phase]*np.diagonal(M)+(1/4)*(U_IC[:,:,phase]*np.diagonal(M)-dt*dU_ic[:,:,phase]))/np.diagonal(M_n)

def update_interface_val3_3(dt,dU_ic):
    # 三阶格式的第三步
    for phase in range(2):
        x_int_IC = 0.5 * IC_vol[phase] * x_int_ref + IC_centroid[phase]
        ele_bias_IC = bais(x_int_IC, IC_centroid[phase], IC_vol[phase])
        ...
        M = (ele_bias_IC * wi) @ np.transpose(ele_bias_IC) * IC_vol[phase]
        M_n = (ele_bias_IC_n * wi) @ np.transpose(ele_bias_IC_n) * IC_vol_new[phase]
        U_IC[:,:,phase] =((1/3)*U_IC_cp[:,:,phase]*np.diagonal(M)+(2/3)*(U_IC[:,:,phase]*np.diagonal(M)-dt*dU_ic[:,:,phase]))/np.diagonal(M_n)
...
def TVB_limiter2(U_data,U_IC):
    ...
    DU_m[:,IC_idx+2,1] = U_data[:,0,IC_idx+2,1] - U_IC[:,0,1]
    # 单元的边界上施加限制 单边的
    ele_bias_l = bais(point[0:-1], ele_centroid, ele_vol)
    ele_bias_r = bais(point[1:], ele_centroid, ele_vol)
    U_ele_l0 = np.sum(ele_bias_l * U_data[:,:,:,0], axis=1)
    U_ele_l1 = np.sum(ele_bias_l * U_data[:,:,:,1], axis=1)
    U_ele_r0 = np.sum(ele_bias_r * U_data[:,:,:,0], axis=1)
    U_ele_r1 = np.sum(ele_bias_r * U_data[:,:,:,1], axis=1)
    dUr0 = U_ele_r0 - U_data[:,0,:,0]
    dUr1 = U_ele_r1 - U_data[:,0,:,1]
    dUl0 = U_data[:,0,:,0] - U_ele_l0
    dUl1 = U_data[:,0,:,1] - U_ele_l1
    # 在每个单元上分别施加限制器
    for i in range(IC_idx-1):
        ...
        # 施加限制器
        dUr0_mod = R@minmod(R_inv@dUr0[:,i],R_inv@DU_p[:,i,0],R_inv@DU_m[:,i,0],M_para,ele_vol)
        dUl0_mod = R@minmod(R_inv@dUl0[:,i],R_inv@DU_p[:,i,0],R_inv@DU_m[:,i,0],M_para,ele_vol)
        # dUr0_mod = minmod(dUr0[:,i],DU_p[:,i,0],DU_m[:,i,0],M_para,ele_vol)
        # dUl0_mod = minmod(dUl0[:,i],DU_p[:,i,0],DU_m[:,i,0],M_para,ele_vol)
        # 更新的值做替换
        U_data[:,1,i,0] = 0.5*(dUr0_mod[:]+dUl0_mod[:])
        U_data[:,2,i,0] = (3/4)*(dUr0_mod[:]-dUl0_mod[:])
    for i in range(IC_idx+2,n_ele):
        ...
        p = U1d.p
        c = U1d.sound_speed()
        H = (U_data[2,0,i,1]+p)/rho
        R = np.array([[1, 1, 1], [u - c, u, u + c], [H - u * c, 0.5 * u ** 2, H + u * c]])+eps*np.eye(3)
        R_inv = np.linalg.inv(R)
        dUr1_mod = R@minmod(R_inv@dUr1[:,i],R_inv@DU_p[:,i,1],R_inv@DU_m[:,i,1],M_para,ele_vol)
        dUl1_mod = R@minmod(R_inv@dUl1[:,i],R_inv@DU_p[:,i,1],R_inv@DU_m[:,i,1],M_para,ele_vol)
        U_data[:,1,i,1] = 0.5*(dUr1_mod[:]+dUl1_mod[:])
        U_data[:,2,i,1] = (3/4)*(dUr1_mod[:]-dUl1_mod[:])
        # 施加限制器
    # 界面单元上单独的限制器
    U0IC0 = U_IC[:,0,0]
    U0IC1 = U_IC[:,0,1]
    ele_bias_l_IC0 = bais_single(point[IC_idx-1], IC_centroid_new[0], IC_vol_new[0])
    ele_bias_r_IC0 = bais_single(x_I_new, IC_centroid_new[0], IC_vol_new[0])
    ele_bias_l_IC1 = bais_single(x_I_new, IC_centroid_new[1], IC_vol_new[1])
    ele_bias_r_IC1 = bais_single(point[IC_idx+2], IC_centroid_new[1], IC_vol_new[1])
    U_IC0_l = np.sum(ele_bias_l_IC0 * U_IC[:,:,0], axis=1)
    U_IC0_r = np.sum(ele_bias_r_IC0 * U_IC[:, :, 0], axis=1)
    U_IC1_l = np.sum(ele_bias_l_IC1 * U_IC[:,:,1], axis=1)
    U_IC1_r = np.sum(ele_bias_r_IC1 * U_IC[:,:,1], axis=1)
    dUr0 = U_IC0_r - U0IC0
    dUl0 = U0IC0 - U_IC0_l
    dUr1 = U_IC1_r - U0IC1
    dUl1 = U0IC1 - U_IC1_l
    # 施加加界面单元上的限制器
    U1d_IC0 = Fd1d.Fluid_1d(U0IC0,gamma=gamma0)
    rho = U1d_IC0.rho
    u = U1d_IC0.u
    p = U1d_IC0.p
    c = U1d_IC0.sound_speed()
    H = (U_IC[2,0,0]+p)/rho
    R = np.array([[1, 1, 1], [u - c, u, u + c], [H - u * c, 0.5 * u ** 2, H + u * c]])+eps*np.eye(3)
    R_inv = np.linalg.inv(R)
    dUr0_mod = R@minmod(R_inv@dUr0[:],R_inv@DU_p[:,IC_idx-2,0],R_inv@(U0IC1-U0IC0),M_para,IC_vol_new[0])
    dUl0_mod = R@minmod(R_inv@dUl0[:],R_inv@DU_p[:,IC_idx-2,0],R_inv@(U0IC1-U0IC0),M_para,IC_vol_new[0])
    U_IC[:,1,0] = 0.5*(dUr0_mod[:]+dUl0_mod[:])
    U_IC[:,2,0] = (3/4)*(dUr0_mod[:]-dUl0_mod[:])
    U1d_IC1 = Fd1d.Fluid_1d(U0IC1,gamma=gamma1)
    rho = U1d_IC1.rho
    u = U1d_IC1.u
    p = U1d_IC1.p
    c = U1d_IC1.sound_speed()
    H = (U_IC[2,0,1]+p)/rho
    R = np.array([[1, 1, 1], [u - c, u, u + c], [H - u * c, 0.5 * u ** 2, H + u * c]])+eps*np.eye(3)
    R_inv = np.linalg.inv(R)
    dUr1_mod = R@minmod(R_inv@dUr1[:],R_inv@(U0IC1-U0IC0),R_inv@DU_m[:,IC_idx+2,1],M_para,IC_vol_new[1])
    dUl1_mod = R@minmod(R_inv@dUl1[:],R_inv@(U0IC1-U0IC0),R_inv@DU_m[:,IC_idx+2,1],M_para,IC_vol_new[1])
    U_IC[:,1,1] = 0.5*(dUr1_mod[:]+dUl1_mod[:])
    U_IC[:,2,1] = (3/4)*(dUr1_mod[:]-dUl1_mod[:])

def TVB_limiter2_subcell(U_data):
    #在子单元上施加限制器
    # P2 元的限制器
    # P2 元的限制器
    DU_p= np.zeros([DIM+2,n_ele,2],dtype=float)
...
if __name__ == '__main__':
    ...
    while (not is_stop):
        ...
        u_star = oneD_Riemann(U_IC[:, 0, 0], U_IC[:, 0, 1])
        dU, dUIC, max_speed = get_flux(U_data, max_speed)
        x_I_new = (3/4)*x_I_cp+(1/4)*(x_I+u_star*dt)
        phi_new = point - x_I_new
        IC_idx_new = interface_cell_idx_new(phi_new)
        update_phase_info_new(x_I_new)
        IC_vol_new, IC_centroid_new = update_interface_info_new()
        U_data = (3/4) * U_data_cp + (1/4) * (U_data - dt * dU)
        update_interface_val3_2(dt, dUIC)
        TVB_limiter2(U_data, U_IC)
        decompose_interface_val()
        updata_val_backup(phase_vol_cp, phase_centroid_cp)
        phi = phi_new
        ...
        u_star = oneD_Riemann(U_IC[:, 0, 0], U_IC[:, 0, 1])
        dU, dUIC, max_speed = get_flux(U_data, max_speed)
        x_I_new = (1/3)*x_I_cp+(2/3)*(x_I+u_star*dt)
        phi_new = point - x_I_new
        IC_idx_new = interface_cell_idx_new(phi_new)
        update_phase_info_new(x_I_new)
        IC_vol_new, IC_centroid_new = update_interface_info_new()
        U_data = (1/3) * U_data_cp + (2/3) * (U_data - dt * dU)
        update_interface_val3_3(dt, dUIC)
        TVB_limiter2(U_data, U_IC)
        decompose_interface_val()
        phi = phi_new
        x_I = x_I_new
