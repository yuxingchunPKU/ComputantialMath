...
def compose_interface_val_backup():
    #组合备份界面单元上的值
    for phase in range(2):
        Uf = np.zeros([DIM+2,M_b,3],dtype=float)
        for i in range(IC_idx-1,IC_idx+2):
            ...
            if phase_vol[phase,i] > eps:
                ...
                ele_bias_sc = bais(x_int_sc, phase_centroid[phase,i], phase_vol[phase,i])
                ele_bias_sc_IC = bais(x_int_sc, IC_centroid[phase], IC_vol[phase])
                Uf[:,:,i-(IC_idx-1)] = (U_data_cp[:,:,i,phase] @ ele_bias_sc) @ np.transpose(ele_bias_sc_IC*wi) * phase_vol[phase,i]
        x_int_IC = 0.5 * IC_vol[phase] * x_int_ref + IC_centroid[phase]
        ele_bias_IC = bais(x_int_IC, IC_centroid[phase], IC_vol[phase])
        M = (ele_bias_IC * wi) @ np.transpose(ele_bias_IC) * IC_vol[phase]
        U_IC_cp[:,:,phase] = np.sum(Uf,axis=2)/np.diagonal(M)
...
def decompose_interdace_val_backup():
    #分解备份界面单元上的值到普通单元上去
    for phase in range(2):
        for i in range(IC_idx - 1, IC_idx + 2):
            # 取出总体的基函数 然后分配到单元上去
            if phase_vol_new[phase, i] > eps:
                ...
                ele_bias_sc = bais(x_int_sc, phase_centroid_new[phase, i], phase_vol_new[phase, i])
                ele_bias_sc_IC = bais(x_int_sc, IC_centroid_new[phase], IC_vol_new[phase])
                F_UIC = (U_IC_cp[:,:,phase] @ ele_bias_sc_IC) @ np.transpose(ele_bias_sc * wi) * phase_vol_new[phase, i]
                Mi = (ele_bias_sc * wi) @ np.transpose(ele_bias_sc) * phase_vol_new[phase, i]
                U_data_cp[:,:,i,phase] = F_UIC / np.diagonal(Mi)
            else:
                U_data_cp[:,:,i,phase] = 0


def update_interface_val_cp(dt,dU_ic):
    for phase in range(2):
        x_int_IC = 0.5 * IC_vol[phase] * x_int_ref + IC_centroid[phase]
        ele_bias_IC = bais(x_int_IC, IC_centroid[phase], IC_vol[phase])
...
def update_interface_val3_2():
    # 三阶格式的第二步
    pass

def update_interface_val3_3():
    # 三阶格式的第三步
    pass
...
def TVB_limiter1(U_data,U_IC):
    ...
    # 周期边界条件
    DU_p[:,0,:] = U_data[:,0,0,:]-U_data[:,0,-1,:]
    DU_m[:,-1,:] = U_data[:,0,0,:]-U_data[:,0,-1,:]
    #靠近界面单元上的值 单独处理
    ...
    # 代入限制器函数
    dUp_IC0_mod = minmod(dUp_IC0,DU_p[:,IC_idx-2,0],DU_p[:,IC_idx-2,0],0,IC_vol_new[0])
    dUp_IC1_mod = minmod(dUp_IC1,DU_m[:,IC_idx+2,1],DU_m[:,IC_idx+2,1],0,IC_vol_new[1])
...
if __name__ == '__main__':
    ...
    while (not is_stop):
        ...
        #update_interface_val_cp(dt,dUIC)
        # 或者是改为界面解除后施加限制器
