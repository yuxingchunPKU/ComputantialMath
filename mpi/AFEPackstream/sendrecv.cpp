...
int main(int argc, char *argv[])
{
  ...
  std::vector<patch_value_entry> my_pv;
  srand(rank);
  my_pv.resize(rand() % n_rank);
  for (int i = 0; i < my_pv.size(); ++i)
  {
    srand(rank + i * n_rank);
    my_pv[i].vol = rand() % n_rank;
    std::fill(my_pv[i].val.begin(), my_pv[i].val.end(), double(rand() % n_rank) / n_rank);
  }
  // 传递序列
  for (int i = 0; i < my_pv.size(); ++i)
  {
  ...
  for (int i = 0; i < n_rank; ++i)
  {
    ...
    for (int k = 0; k < int(buf_size_vec[i] / 40); ++k)
    {
